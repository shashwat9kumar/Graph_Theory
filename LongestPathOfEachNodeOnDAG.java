...
class LongestPathOfEachNodeOnDAG {
    static int vertex = 8;
    ...
    public static void main(String args[]){
        ...
        Arrays.fill(leastDistance, 2147483641);   // -ve value 
        ...
        addEdge(graph, 0, 1, -3);  // Add the negated value of weight of edge
        addEdge(graph, 0, 2, -6);
        addEdge(graph, 2, 3, -6);
        addEdge(graph, 1, 2, -4);
        addEdge(graph, 1, 3, -4);
        ...
        for(int i=0;i<vertex;i++) {
            System.out.print((-leastDistance[ordering[i]]) + "\t-->"); // Print the negated value of distance
        }
        System.out.println();
    ...
    static void dijkstra(ArrayList<ArrayList<Integer>> graph, int startNode) {
        for(int i=0;i<ordering.length;i++) {
            ...
            for(int j=0;j<graph.get(node).size();j++) {
                ...
                if(leastDistance[childNode] > (leastDistance[node] + adMat[node][childNode])) {  // Comparison for highest path
...
A little smarter approach : checking with -ve infinity 
